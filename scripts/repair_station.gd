...
func receive_minigame_input(input_val: String) -> void:
	...
	if minigame_type == "password" and role == "B":
		var pwd = paired_station.mg_state.get("password", [])
		var idx = mg_state.get("input_idx", 0)
		if idx < pwd.size():
			if pwd[idx] == input_val:
				mg_state["input_idx"] = idx + 1
				SoundManager.play(get_tree(), "click", 0.2)
				if mg_state["input_idx"] >= pwd.size():
					fix_station()
			else:
				mg_state["input_idx"] = 0 # Errou, reseta
...
func update_minigame_ui() -> void:
	...
	if minigame_type == "password":
		if role == "A":
			txt = "Senha:\n" + str(mg_state.get("password", []))
		else:
			txt = "Insira a Senha\nProgresso: " + str(mg_state.get("input_idx", 0)) + "/4"
	elif minigame_type == "item":
		if role == "A":
